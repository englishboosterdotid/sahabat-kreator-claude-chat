...
export const getUpcomingMomentumAction = withWorkspacePermission(
  ["owner", "admin", "member", "viewer"],
  async (ctx, daysAhead: number = 30): Promise<MomentumEvent[]> => {
    ...
    const preference = await db.query.workspaceMomentumPreference.findFirst({
      where: eq(workspaceMomentumPreference.workspaceId, ctx.teamId),
    });
    ...
    // Tambahkan tanggal gajian bulan ini & bulan depan (recurring, bukan dari tabel indonesia_momentum)
